...
$('#{{$TableName}}_save').click(function() {
  ...
  $.post(url_address, data, function(response) {
    ...
    $('#document_type_id').append($("<option></option>").attr("value",response.key).text(response.value));
    $('#{{$TableName}}Form')[0].reset();
    $('#document_type_id').val(response.key).trigger('change');
    $('#{{$TableName}}Modal').modal('hide');
    var routeName="{{Route::getCurrentRoute()->getActionMethod()}}";
    if(routeName=='DocumentType'){
      dataTable.fnDraw();
    }
  }, "json");
});
